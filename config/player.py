...
class Player:
    ...
    def draw_with_sword(self, surf, px, py, player_img, sword_img, sword_slash_imgs):
        ...
        if self.sword_swinging and sword_slash_imgs:
            ...
            # Offset sword to player's hand (tweak as needed)
            offset_x = -10 if self.facing_left else 20
            offset_y = 10
            if self.facing_left:
            ...
            offset_y = 10
            surf.blit(player_img, (px, py))
            surf.blit(sword_img, (px + offset_x, py + offset_y))
    x: float
    y: float
    w: int = 28
    h: int = 36
    ...
    sword_anim_speed: float = 0.06  # seconds per sword frame
